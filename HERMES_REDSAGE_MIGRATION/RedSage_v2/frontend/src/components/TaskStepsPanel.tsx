...
import type { StepVerificationResponse, TaskStatus, TaskStepSummary } from '../types';
...
export default function TaskStepsPanel({ projectId, task, locked, onChanged, onOpenMentor, onError }: {
  ...
  locked: boolean;
  onChanged: () => void;
  onOpenMentor: (step: TaskStepSummary) => void;
  ...
  const [verdict, setVerdict] = useState<StepVerificationResponse>();
  const steps = (task.steps ?? []) as TaskStepSummary[];
  ...
  async function transition(step: TaskStepSummary, status: TaskStatus) {
  ...
  async function verifyStep(step: TaskStepSummary) {
    if (!raw.trim() || verifying) return;
    setVerifying(true);
    setVerdict(undefined);
    try {
      const result = await apiCall<StepVerificationResponse>(`/projects/${projectId}/tasks/${task.id}/steps/${step.id}/verify`, {
        method: 'POST',
  ...
  function toggle(step: TaskStepSummary) {
  ...
  return <section className="task-steps">
    ...
    {steps.map((step: TaskStepSummary) => <div className="task-step-block" key={step.id}>
      <div className="task-step">
        <div><strong>{step.title}</strong><small>{step.status.replaceAll('_', ' ')}</small></div>
        <select value={step.status} onChange={(event) => transition(step, event.target.value as TaskStatus)}>
