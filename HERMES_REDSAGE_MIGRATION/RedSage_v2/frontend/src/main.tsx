...
import type { EvidenceItem, TaskVerificationResponse } from './types';
import './index.css';
import './day2.css';
import './day3.css';
import './brief.css';
...
export default function App() {
  ...
  const [verdict, setVerdict] = useState<TaskVerificationResponse>();
  ...
  async function verify() {
    ...
    setVerifying(true);
    try { const result = await apiCall<TaskVerificationResponse>(`/projects/${project.id}/tasks/${selected.id}/verify`, { method: 'POST', body: JSON.stringify({ raw_content: rawEvidence }) }); setVerdict(result); setRawEvidence(''); await refresh(); }
    catch (err: any) { setError(err.message); }
